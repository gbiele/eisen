...
date: "31 august 2018"
output: pdf_document
>>>>>>> abc65ac5285fa539a5342ab3d903ead66e14fce2
---
...
=======
Next we do log transformations for slected variables to get rid of the right-skewed values. We are also plotting histograms before and after  transformation for the variables we transform. 
>>>>>>> abc65ac5285fa539a5342ab3d903ead66e14fce2
...
=======
```{r}
large_cors = names(which(colSums(abs(cm) > .9) > 0))
hcrs = cm[large_cors,large_cors]
heatmap.2(hcrs)
>>>>>>> abc65ac5285fa539a5342ab3d903ead66e14fce2
```
...
=======
Now we prepare data for a model with automatic shrinkage. 
The special challange here is that we want to use quadratic terms (and later maybe all pairwise interactions) while also imputing missing data.
Hence, we have a somewhat elaborate procedure to pre-process the data and prepare the complete design matrix in the Stan model itself.

To reiterate: Outside Stan we only generate a design matrix with columns for each basic variable. Inside Stan we generate the quadratic (and interaction) terms once for all data and then at each iteration of the sampling process again for all imputeted variables.


We start by scaling variables and saveing scaling values. The matrix preX that is prepared below is the basic design matrix that includes all variables which will be log-transformed, scaled, and used to generate quadratic terms for numeric variables in Stan.
>>>>>>> abc65ac5285fa539a5342ab3d903ead66e14fce2
...
=======
To speed up the fitting, we remove interaction variables that have a low entropy or are highly correlated with other (non-iteraction) variables. 
>>>>>>> abc65ac5285fa539a5342ab3d903ead66e14fce2
...
=======
                ...
                slab_scale = .5,
                N_missing = N_missing,
                N_has_missing = N_has_missing,
                miss_row = idx_miss_interacts[,1],
                miss_col = idx_miss_interacts[,2],
                M_quadratic = M_quadratic,
                scale_sd = scale_sd,
                scale_mu = scale_mu,
                transf_idx = range(transf_idx),
                ...
                N_missing_interactions = N_missing_interactions,
