...
## The Stan model
This is a basic version of the model that doe snot include interations yet. I'll do this when I have some spare time.
...
This are the 20 most important predictors. Note that variables that and in 2 are quadratic terms and variables that end in _b are binary variables derived from zero-inflated numeric variables.

```{r}
plot_beta(beta[1:20,])
```

Thats it for now. We see that we get an $R^2$ that is OK but not great with this basic model. I can think of at least 3 things to do to approach a good prediction model:
